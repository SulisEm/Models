globals [
 ...
 month year  ; MONITORS: from 0 (2016)
 tot-pat-each-day   ;; tot patients
 ...
 count-discharged-dom count-discharged-hos count-discharged-aba count-discharged-tra count-discharged-dea    ;; discharged
 kpi-DTDT kpi-TRD num-discharged             ;; KPIs
 n-p
]
...
to start
  ...
  check-discharged       ; to control patients with exit? = True
...
to add-to-wl
  ifelse nextward = 105 [ set wl-RA lput who wl-RA  ]
  [ ifelse nextward = 65 [ set wl-TR lput who wl-TR  ]
    ...
    [
      ...
      [
        ifelse not pat-in-waiting-list?
          [
            if nextWard = 95     ;; OBI - Observation
              ...
              [
                ...
                [ ifelse exam:RX?               ;;; RX
                  ...
                  [ ifelse exam:ECO?            ;;; ECO
                    [ set nextWard 124 set exam:ECO? False ]
                    [
                      ifelse random 10 < 3 [ set nextWard 95 ][ set nextWard 0 ]  ;;; DECISION : 1 over 5 remains in Observation
                    ]
                  ]
                ]
              ]
            ...
            if nextWard = 30
              [ set count-discharged-dea count-discharged-dea + 1 die ]
...
to-report  pazienti-nella-rispettiva-lista-di-attesa [ rep ]
  ifelse rep = 105 [ report length wl-RA > 0 ]
  [ ifelse rep = 65 [ report length wl-TR > 0 ]
...
to-report set-time-of-activity [ colore ]
 if colore = 105 [ report 30 + random (60) ]       ; registration -> 1 min =  60 seconds/ticks
...
to check-wards
  ...
  [ ?1 ->
    ask ?1  ;;; When a patient arrive (move? False) on a patch, and operators are OK > > >  execute the activity (set: patient served? + operators busy? + times + patch free )
  ...
  if any? patients with [exit? = True]
    [ ask patients with [exit? = True]
      [ set t-of-dismission ticks
        ...
        let t-r-d 0
        set num-discharged   num-discharged + 1

        set t-r-d (t-of-dismission - t-of-registration) / 60
        set kpi-TRD ((kpi-TRD * ( num-discharged - 1)) + t-r-d ) /  num-discharged

        if outcome = "dea" [set count-discharged-dea count-discharged-dea + 1]
        if outcome = "hos" [set  count-discharged-hos count-discharged-hos + 1]
        if outcome = "dis" [set count-discharged-dom count-discharged-dom + 1]
        if outcome = "tra" [set count-discharged-tra count-discharged-tra + 1]
        if outcome = "abb" [set count-discharged-aba count-discharged-aba + 1]
  ...
  [
    ...
    [
      foreach sort operators with [ not busy? and not op-in-waiting-list? and not moveOp?]
        [ ?1 ->
          ask ?1 [ insert-operator-in-waiting-list [who] of ?1 [profession] of ?1 ]
        ]
    ]
  ]
end

to check-patients    ; check for patients on an activity
    ...
    [
      ...
      [ ask patch-here
        [
          ...
          if operators-needed = 0 [set waiting-for-operators? False ]
        ]
        set moveOp? False
      ]
      [ walk-to-next-ward ]
...
to check-activity-free
  let i  0
  foreach sort patches with [activity?]
  [ ?1 ->
    ask ?1
    [ ; Insert free activity into list-activity-free (if and when they become free)
      if free? and not ward-in-waiting-list?
      [
        ...
        [
          if verifica-operatori-liberi-nella-rispettiva-lista-di-attesa pcolor
            [
              ...
              set list-activity-free remove ?1 list-activity-free
              set ward-in-waiting-list? False
...
to re-create-patients  ;; every day, generate a new distribution
  if ticks mod 86400 = 0 [setup-patients]
  if not any? ambulances [setup-ambulances]
end

to import-map
  import-drawing "ED-NEW-MAP.png"
end

to setup-actual-ED
  import-drawing "ED-OLD-MAP.jpg"
end
...
to setup
  ...
  set num-discharged 0 set count-discharged-dom 0 set count-discharged-aba 0 set count-discharged-hos 0 set count-discharged-tra 0 set count-discharged-dea 0
end
...
to setup-operators   ;; create and setup operators
  ...
  [ ?1 -> ask ?1
    [
      insert-operator-in-waiting-list [who] of ?1 [profession] of ?1
...
to setup-patients   ;; create patients for one day
  ...
  (foreach h-of-day distArrivi [ [?1 ?2] ->
...
327
50
Setup the simulation (choose an ED)
16
103.0
1
...
At the beginning ("SETUP" button), the ED activities become the world of the simulation. Different colors correspond to different ED wards. Activities without patients are "free" and wait (into a "list-activity-free") to call to them patients and operators, once both of them are waiting for that activity.
