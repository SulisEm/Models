breed [people customer]
breed [operators operator]

people-own [
  state            ;; "M": Moving (to the next Task/Queue) - "W": Waiting (on the Task)  - "S": Service (having the Service/Task)
  next-task        ;; name of the next task (performing the corresponding service)
  destination      ;; the task where the agent has to move
  arrival-time     ;; the ticks/time at the arrival
  work-durat-task  ;; the amount of time/duration of the work on the task
]

operators-own  [
  state            ;; "L": Looking - "M": Moving - "W": Waiting - "S": Service
  work-with        ;; the agent that the operator will serve (or to work with)
...
globals [
  monit-discharged  ;; monitor to show discharged patients
]
...
to go
  ...
  if ticks = 28800 [ stop ]   ;; stop condition (8 hours => 8 * 60 * 60 seconds)
  arrival-tokens              ;; create new arrivals/tokens
  move-agents                 ;; move people
  check-agent-in-queue        ;;
  start-automatic-tasks       ;; checking for a customer on an automatic task: if yes, they start the service "S"
  start-human-tasks           ;; check if both operator and customer/agent/token are waiting on a (human) task: if yes, they start the service "S"
  operators-looking           ;;
  check-time
end
...
to setup-world
  ...
  ;; initialize each task with their type ("AutomaticTask", "HumanTask" or "queue") / name (name of the patch) / next-task-of-queue ()
  ;; / position (absolute coordinates pxcor and pycor) / (background) color of the patch / duration times
  ask patches with [ pxcor = -12 and pycor = 9 ] [ set pcolor 8 set name "Arrival customer" set typeForm "task" ]
  ask patches with [ pxcor = 0  and pycor = 10 ] [ set pcolor 38 set name "Queue-TakeNumber" set typeForm "queue" ]
  ask patches with [ pxcor = 12  and pycor = 9 ] [ set pcolor 78 set name "Take number" set duration-triang "" set duration-mean 3 set duration-devst 1 set typeForm "AutomaticTask" set name-queue "Queue-TakeNumber"]
  ask patches with [ pxcor = 13  and pycor = 1 ] [ set pcolor 38 set name "Queue-Registration" set typeForm "queue" ]
  ask patches with [ pxcor = 12 and pycor = -7 ] [ set pcolor 97 set name "Registration" set duration-triang "10*12*30" set typeForm "HumanTask" set name-queue "Queue-Registration"] ;"2.5*5*9.5"
  ask patches with [ pxcor = 4  and pycor = -4 ] [ set pcolor 38 set name "Queue-TicketPayment" set typeForm "queue" ]
  ask patches with [ pxcor = -4 and pycor = -4 ] [ set pcolor 78 set name "Ticket payment" set duration-triang "20*30*65" set typeForm "AutomaticTask" set name-queue "Queue-TicketPayment"] ;2.5*3*3.5"
  ask patches with [ pxcor = -4  and pycor = -10 ] [ set pcolor 38 set name "Queue-Operation" set typeForm "queue" ]
  ask patches with [ pxcor = -12 and pycor = -10 ] [ set pcolor 97 set name "Operation" set duration-triang "10*20*60" set typeForm "HumanTask" set name-queue "Queue-Operation"] ; "2*4*6" or set duration-mean 900 set duration-devst 10
...
to setup-operators
  ;; create a number of operators having a shape of "person", a state "L", inizializing the duration variable
  create-operators n-of-operators [ setxy 0 3 set shape "person" set state "L" set work-durat-task 0 ]
end

to setup-monitors
  set monit-discharged 0
end

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;  PROCEDURE TO COMPUTE THE FLOW  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
to-report compute-next-task [ actual-task ]
  ...
  if actual-task = "Registration" [
    ifelse random 100 < %-ticket-payments [ set nt "Ticket payment" ]
    [ set nt "Operation" ]
  ]
...
to arrival-tokens
  if ticks mod 60 = 0 [  ; create-customers every 60 ticks
     create-customers
  ]
end

to create-customers
  create-people n-of-arrivals [  ;; one token every 60 ticks (e.g., each 60 seconds)
    ...
    insert-in-queue self next-task           ;; add 'customer' who into the corresponding list (queue variable)

    set destination one-of patches with [ name = [name-queue] of [next-task] of myself ] ;; at the start, they move to "Queue-TakeNumber"
  ...
  if any? turtles with [state = "M"] [
    ask turtles with [state = "M"] [
      ...
      if patch-here = destination [           ;; once arrived at destination...
        ...
        if breed = people and [name] of next-task = "end" [  ;; at the end of the path
...
to update-dis
  set monit-discharged monit-discharged + 1
end
...
to start-automatic-tasks
  if any? people with [ state = "W" and  [typeform] of patch-here = "AutomaticTask" ][
    ask people with [ state = "W" and  [typeform] of patch-here = "AutomaticTask" ][
      set state "S"
...
to start-human-tasks
  ...
  ask operators with [ state = "W" and [state] of work-with =  "W"  and [typeform] of patch-here = "HumanTask" ]
    [
      set state "S"
...
to operators-looking
  if any? operators with [ state = "L" ] and any? patches with [ typeForm = "HumanTask" and not empty? queue  ] [
    ; RANDOMLY SELECT AN OPERATOR
    ask one-of operators with [ state = "L"] [
      ask one-of patches with [  typeForm = "HumanTask" and not empty? queue ]
...
to check-time
  ...
  ;; PROCEDURE TO CHECK CUSTOMERS
  if any? people with [ state = "S" and work-durat-task = ticks ][
    ask people with [ state = "S" and work-durat-task = ticks ][
      set state "M"
      ask patch-here [ set pstate "free" ]
      ifelse [name] of next-task != "Operation" [
        set next-task compute-next-task [name] of next-task
        insert-in-queue self next-task               ; insert token into queue
...
The model describes the operation of a service (e.g., registration to a hospital laboratory) with four activities: two of these involve an operator, the other two are performed by clients (patients).

## HOW IT WORKS

Agents (both operators and customers) act accordingly to their variable State 
Activities can be human tasks or automatic tasks.

## HOW TO USE IT

Basic model to simulate a simple registration process

## THINGS TO NOTICE

Duration of activity computed with a triangular distribution ("z-random-triangular"), according to duration-triang patch variable.
