...
tokens-own [
  state            ;; "M": Moving (to the next Task/Queue) - "W": Waiting (on the Task)  - "S": Service (doing the Service, on the Task)
...
to setup-world
  ...
  ;; initialize each task with their type ("AutomaticTask", "HumanTask" or "queue") / name (name of the patch) / next-task-of-queue () / position (absolute coordinates pxcor and pycor) / (background) color of the patch / duration times
