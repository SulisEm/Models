...
patches-own [ reward Qlist name l ]

breed [ cars car ]

globals[ave-reward r-episode episode bestProcess bestProcesses num-run num-act torino ]

cars-own [Qsa Hlist process search-for]

to setup
  ca
  ;set the basic ground / map
  setup-geo
  setup-ground

  ;--patches are the individual states: set up color, reward, and inital Q(s,a) values
  ask patch -18 -25 [set pcolor  green - 2] ;starting point
  ...
  create-cars 1
  [ set shape "car" set color red - 2 setxy -18 -25 set size 1 set Hlist [90 180 270 0]  ;;if pen-down? [pen-down]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
to setup-geo
  ask patches [ set pcolor 39 set plabel-color black ]

  ;import GIS shapefile
  set torino gis:load-dataset "gis-to/carta_sintesi_geo.shp"
  gis:set-drawing-color white
  gis:draw torino 0.3
  gis:apply-coverage torino "LAYER" l

  ;let's color different elements of the map
  ask patches with [l = "FIUMI"] [set pcolor blue]
  ask patches with [l = "PARCHI"] [set pcolor 63]
  ask patches with [l = "ISOLATI"] [set pcolor gray]
  ask patches with [l = "PONTI"] [set pcolor gray]
end
...
to import-process-info
  ...
  while [i < num-stops] [
    ask one-of patches with [pcolor = gray and (distancexy -31 -55 < 40) ] [set name item i activities]
    set i i + 1
  ]
end
...
to set-start
  let s item num-act [search-for] of car 0
...
to setup-rewards
  ...
  set-start
  ask patches [if name != 0 [set plabel name ]]
end
...
to go
  ...
  while [num-act < num-stops] [
    ...
    while [num-run <= num-runs] [
      ...
      ;; best process
      let p [process] of car 0

      if length bestProcess = 0 or length bestProcess > length p [;item num-act
        set bestProcess p
      ]
      init-run
    ...
    ask car 0 [ move-to item (length bestProcess - 1) bestProcess ]
